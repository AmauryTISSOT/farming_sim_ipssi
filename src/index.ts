...
app.patch("/createField", async (req, res) => {
  try {
    ...
    if (!field) {
      return res.status(400).json({ error: "Max field capacity reached." });
    }
...
app.post("/createMachine", async (req, res) => {
  try {
    ...
    return res.status(201).json({ message: "Machines created successfully." });
  } catch (error) {
...
app.post("/initializeWaterTank", async (req, res) => {
  try {
    farmManager.initializeWaterTank();
  } catch (error) {
    console.error("Error initialize water tank :", error);
    return res.status(500).json({ error: "Internal server error" });
  }
});
...
app.get("/storage", (req, res) => {
  ...
  const stock = Array.from(storage.stock.entries()).map(([name, quantity]) => ({
    name,
    quantity,
  }));
  res.json({
